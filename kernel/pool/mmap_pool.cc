...
namespace kernel
{
    namespace pool
    {
        ...
        MMapPool::MMapPool() : begin_(nullptr), min_expand_size_(kDefaultMinExpandSize)
        {
        }
        ...
        bool MMapPool::LoadFreeList(const std::string &free_list_file)
        {
            ...
            for (size_t i = 0UL; i < size; i++)
            {
                ...
                block.addr = GetMMapDataBegin() + reinterpret_cast<size_t>(block.addr);
        ...
        {
            if (read_only_ || free_list_meta_.user_space_size == 0UL)
                return true;
            store::FileStore *store = new store::FileStore();
            if (store == nullptr)
            {
                 ...
                 it++)
            {
                ...
                while (!list.empty())
                {
                    ...
                    size++;
                    size_t offset = reinterpret_cast<char *>(block.addr) - GetMMapDataBegin();
